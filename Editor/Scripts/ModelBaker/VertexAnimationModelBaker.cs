...
namespace TAO.VertexAnimation.Editor
{
	...
	public class VertexAnimationModelBaker : ScriptableObject
	{
		...
		[SerializeField] private VaPrebakeAnimationCollection collection_s;		
		[Range(1, 60)]
		public int fps = 24;
		...
		public List<HumanBodyBones> explosedBones=new List<HumanBodyBones>();
		// Output.
		...
		public void Bake()
		{
			...
			AnimationBaker.BakedData bakedData = target.Bake(collection_s.prebakeCollection, applyRootMotion, fps, textureWidth,explosedBones);
		...
		private void GenerateBook(AnimationBaker.BakedData bakedData)
		{
			...
			{
				string animationName = $"{name}_{info[i].name}";
				VA_AnimationData newData = new VA_AnimationData()
				{
					name = animationName,
					frames = info[i].frames,
					maxFrames = info[i].maxFrames,
					animationMapIndex = i,
					colorMapIndex = -1,
					frameTime = 1.0f / info[i].maxFrames * info[i].fps,
					duration = 1.0f / info[i].maxFrames * (info[i].frames - 1),
				};

				// Either update existing animation or create a new one.
				if (TryGetAnimationWithName(animationName, out var animation))
				{
